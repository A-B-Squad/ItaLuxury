...
const ProductInfo = memo(({
  ...
  // Initialize and update viewer count
  useEffect(() => {
    // Generate initial viewer count based on product ID for consistency
    const hash: number = productDetails.id.split('').reduce((acc: number, char: string) => acc + char.charCodeAt(0), 0);
    const baseCount: number = Math.abs(hash) % 25 + 8; // Between 8-32 viewers (more realistic)

    setViewerCount(baseCount);

    // Simulate realistic viewer count changes
    const updateViewers = (): void => {
      setViewerCount((prev: number) => {
        // More realistic: smaller changes, sometimes no change
        const random: number = Math.random();
        let change: number = 0;

        if (random < 0.3) {
          change = -1; // 30% chance to decrease by 1
        } else if (random < 0.5) {
          change = 0; // 20% chance no change
        } else if (random < 0.85) {
          change = 1; // 35% chance to increase by 1
        } else {
          change = Math.floor(Math.random() * 3) + 1; // 15% chance to increase by 1-3
        }

        // Keep between 3-45 viewers (more realistic range)
        const newCount: number = Math.max(3, Math.min(45, prev + change));
        return newCount;
      });
    };

    // Random interval between 5-12 seconds (more frequent, realistic)
    const scheduleNextUpdate = (): NodeJS.Timeout => {
      const delay: number = Math.random() * 7000 + 5000;
      return setTimeout(() => {
        updateViewers();
    ...
    let timeoutId: NodeJS.Timeout = scheduleNextUpdate();
  ...
  return (
    <div className="productInfo relative grid- w-full  p-3 md:p-5 bg-white rounded-lg shadow-sm">
      <div className="flex flex-col  border-b border-gray-200 pb-2">
        ...
        <div className="mt-2 p-3 bg-gray-50 border border-gray-200 mb-2 rounded-md">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-gray-600">
              <span className="font-semibold text-gray-800 transition-all duration-500">{viewerCount}</span>
              {' '}personne{viewerCount > 1 ? 's' : ''} regardent ce produit en ce moment
            </p>
          </div>
        </div>
