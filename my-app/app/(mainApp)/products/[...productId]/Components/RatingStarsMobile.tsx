    ...
    return (
        ...
        <div className=" block lg:hidden">
            {showRatingSection ? (
                <>
                    <div
                        className="flex items-center cursor-pointer tracking-wider hover:opacity-80 transition-all text-black"
                        onClick={() => setShowRatingSection(false)}
                    >
                        <CiStar size={22} className="mr-2" />
                        <span>Fermer votre avis</span>
                    </div>
                    <div className="flex flex-col items-center mt-6 space-y-4 w-full max-w-3xl mx-auto bg-white rounded-lg shadow-md p-3">
                        {/* Star Rating Section */}
                        ...
                        <div className="w-full pt-1 border-t border-gray-100">
                            ...
                            <div className="space-y-3">
                                {[5, 4, 3, 2, 1].map((rating) => {
                                    const ratingKey = ["one", "two", "three", "four", "five"][5 - rating];
                                    const percentage = reviews > 0 ? ((ratings[ratingKey as keyof typeof ratings] / reviews) * 100).toFixed(1) : 0;
