...
const DELIVERY_COST = 8; // 8 TND delivery cost
  ...
  const trackWhatsAppPurchase = async () => {
    try {
      ...
      triggerEvents("Purchase", {
        user_data: {
          em: [formData.name.toLowerCase().replace(/\s+/g, '') + '@whatsapp.com'],
          fn: [formData.name],
          ph: [formData.phone],
          country: ["tn"],
          ct: formData.governorate,
          external_id: decodedToken?.userId || `wa_${Date.now()}`,
        },
        custom_data: {
