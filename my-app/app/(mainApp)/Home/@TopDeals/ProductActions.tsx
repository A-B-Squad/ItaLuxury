...
const ProductActions = ({ product, toast, isFavorite, openProductDetails }: ProductProps) => {
    const { addToComparison, comparisonList } = useProductComparisonStore();
    const { decodedToken } = useAuth();

    const onAddToCompare = () => {
        const isProductAlreadyInCompare = comparisonList.some((p: any) => p.id === product.id);
    ...
    return (
        <div className="flex items-center space-x-2">
            <QuickActionButton
                ...
                title="Aperçu rapide"
                className="bg-gray-100 hover:bg-gray-200 text-gray-700 transition-transform hover:scale-105"
            />
            ...
            <QuickActionButton
                ...
                className="bg-blue-50 hover:bg-blue-100 text-blue-700 transition-transform hover:scale-105"
            />
            ...
            <FavoriteProductButton
                ...
                className="h-9 w-9 flex items-center justify-center rounded-full transition-transform hover:scale-105"
            />
        </div>
    );
};
