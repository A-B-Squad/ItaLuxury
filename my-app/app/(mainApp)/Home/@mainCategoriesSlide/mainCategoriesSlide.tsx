...
const MainCategoriesSlide = () => {
  ...
  return (
    <Carousel
      className={`carousel w-full  grid   items-center transition-all duration-500 my-5   `}
    >
      <CarouselContent className=" carousel_content h-full divide-x-2  w-full ">
        {mainCategories?.fetchMainCategories?.map(
          (category: any, index: any) => (
            <>
              <CarouselItem
                key={index}
                className={`carousel-item overflow-hidden    group hover:rounded-sm  h-[150px] transition-all relative pb-2 basis-1/3 lg:basis-1/6 flex  flex-col justify-between  items-center    `}
              >
                <Link
