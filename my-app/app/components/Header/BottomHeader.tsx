...
import SearchBar from "./LaptopSearchBar";
...
const BottomHeader = ({ setShowDropdown, isFixed, userData }: any) => {
  ...
  const NavLink = ({ href, onClick, linkId, children }: any) => (
    <Link
      href={href}
      onClick={onClick}
      className={`relative py-2 px-1 font-semibold text-base transition-colors ${activeLink === linkId ? "text-primaryColor" : "text-gray-700 hover:text-primaryColor"
        }`}
    >
      {children}
      {activeLink === linkId && (
  ...
  return (
    <>
      {/* Main Header */}
      <div
        className={`bottom-header-container transition-all duration-300 w-full border-b border-gray-100 ${isFixed
      ...
      >
        ...
        {/* Desktop Header */}
        <div className="desktop-header-wrapper md:block hidden shadow-sm">
          <div className="desktop-header-content flex items-center  px-6 relative">
            {/* Left Section - Categories Button */}
            <div className="desktop-categories-section flex py-2 items-center bg-logoColor text-white">
              <motion.button
                type="button"
              ...
              >
                ...
                <span className="desktop-categories-text font-semibold tracking-wide text-base group-hover:text-primaryColor transition-colors">
                  Nos Catégories
                </span>
              </motion.button>
            </div>
            ...
            <nav className="desktop-navigation LinksDesktop flex-1 h-full flex justify-center">
              <ul className="desktop-nav-list flex h-full items-center gap-8">
                ...
                <li className="nav-item-home h-full">
                  <NavLink
                    href="/"
                    onClick={() => handleNavigation("Page d'accueil", "home")}
                    linkId="home"
                  >
                    Page d&apos;accueil
                  </NavLink>
                ...
                <li className="nav-item-electromenager">
                  <NavLink
                    href="/Collections/tunisie?category=Electroménager&page=1"
                    onClick={() => handleNavigation("Électroménager", "electromenager")}
                ...
                <li className="nav-item-cuisine hidden xl:block">
                  <NavLink
                    ...
                    onClick={() => handleNavigation("Cuisine", "cuisine")}
                    linkId="cuisine"
                  >
                    Cuisine
                  </NavLink>
                ...
                <li className="nav-item-deco hidden xl:block">
                  <NavLink
                    ...
                    onClick={() => handleNavigation("Déco Maison", "deco")}
                    linkId="deco"
                  >
                    Déco Maison
                  </NavLink>
                ...
                <li className="nav-item-coiffure hidden 2xl:block">
                  <NavLink
                    ...
                    onClick={() => handleNavigation("Appareil de coiffure", "coiffure")}
                    linkId="coiffure"
                  >
                ...
                <li className="nav-item-nouveaute hidden 2xl:block">
                  <NavLink
                    ...
                    onClick={() => handleNavigation("Nouveauté", "nouveaute")}
                    linkId="nouveaute"
                  >
                    Nouveauté
                  </NavLink>
            ...
            <div className="desktop-more-menu relative 2xl:hidden ml-auto" ref={moreMenuRef}>
              <motion.button
                onClick={() => setShowMoreMenu(!showMoreMenu)}
                className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors border border-gray-200"
                whileTap={{ scale: 0.95 }}
              >
                <HiPlus className={`text-xl text-gray-700 transition-transform duration-300 ${showMoreMenu ? 'rotate-45' : ''}`} />
                <span className="font-semibold text-base text-gray-700">Plus</span>
              </motion.button>
