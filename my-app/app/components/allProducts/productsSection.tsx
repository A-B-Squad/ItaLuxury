  ...
  return (
    <>
      {loading ? (
        ...
        <div className="w-full grid xl:grid-cols-4 xxl:grid-cols-5 lg:grid-cols-3 px-10 gap-4 md:grid-cols-2 grid-cols-1">
          {products.map((product: Product) => (
            <div key={product.id} className="group my-10 flex w-full max-w-xs flex-col overflow-hidden border border-gray-100 bg-white shadow-md">
              ...
              <div className="mt-4 px-5 pb-5">
                ...
                <div>
                  <p className={`${
                    product?.productDiscounts.length > 0 ? "line-through text-lg" : "text-strongBeige text-xl py-1"
                  } font-semibold`}>
                    {product?.price.toFixed(3)} TND
                  </p>
                  {product?.productDiscounts.length > 0 && (
                    <div className="flex items-center">
                      <span className="text-gray-400 text-xs font-thin">A partir de :</span>
