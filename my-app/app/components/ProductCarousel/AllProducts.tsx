...
const AllProducts = ({
  ...
  return (
    <CarouselItem
      key={productData?.id}
      className="carousel-item  group hover:rounded-sm  h-[420px]   transition-all relative pb-3 flex  flex-col justify-start items-center border shadow-xl basis-full md:basis-1/2  xl:basis-1/5"
    >
      <ul className="plus_button lg:opacity-0 group-hover:opacity-100  absolute right-3 z-50  top-14 flex flex-col gap-3  ">
        ...
        <div
          ...
          onMouseEnter={() => handleMouseEnterHoverPop("Ajouter au comparatif")}
          onMouseLeave={handleMouseLeaveHoverPop}
        ...
        >
          ...
          <li className="bg-strongBeige  rounded-full delay-200 lg:translate-x-20 group-hover:translate-x-0 transition-all p-2 shadow-md hover:bg-mediumBeige ">
            <FavoriteProduct productId={productData?.id} userId={userId} />
          </li>
        </div>
      </ul>
      ...
      >
        ...
        <div className="images relative  -z-10 w-[250px] h-[250px]  transition-all overflow-hidden cursor-crosshair text-black flex justify-center items-center">
          <Image
            src={productData?.images[0]} // Change to select appropriate image
            className="w-full h-full" // Adjust the size of the Image component
